export type VideoInventoryCandidate = {
  id: string
  room: string
  itemName: string
  quantity: number
  ...
  confidence: number
  sourceKind: 'video' | 'transcript' | 'snapshot' | 'mls' | 'photo' | 'manual'
  offsetMs?: number
...
export function normalizeVideoInventoryLabel(value: string) {
...
export function normalizeVideoInventoryRoom(value: string) {
  ...
  if (/garage|shed/.test(room)) return 'garage'
  if (/basement|cellar/.test(room)) return 'basement'
  if (/living|family|great room/.test(room)) return 'living room'
  if (/dining/.test(room)) return 'dining room'
  if (/bed|primary|master/.test(room)) {
    if (room.includes('primary') || room.includes('master')) return 'primary bedroom'
    return roomNumber ? `bedroom ${roomNumber}` : 'bedroom'
  }
  if (/kitchen/.test(room)) return 'kitchen'
  if (/office|study/.test(room)) return 'office'
  if (/storage|locker|container/.test(room)) return roomNumber ? `storage ${roomNumber}` : 'storage'
  if (/patio|yard|outdoor|balcony/.test(room)) return 'outdoor'
  return room || 'unassigned'
}

export function videoInventoryDedupeKey(candidate: Pick<VideoInventoryCandidate, 'room' | 'itemName'>) {
  return `${normalizeVideoInventoryRoom(candidate.room)}:${normalizeVideoInventoryLabel(candidate.itemName)}`
}

export function clusterVideoInventoryCandidates(candidates: VideoInventoryCandidate[], windowMs = 90_000) {
  const sorted = [...candidates].sort((a, b) => Number(a.offsetMs || 0) - Number(b.offsetMs || 0))
  const groups = new Map<string, VideoInventoryCandidate[]>()
...
export function reconcileVideoInventorySources(input: {
  video: VideoInventoryCandidate[]
  transcript?: VideoInventoryCandidate[]
  listing?: VideoInventoryCandidate[]
  photos?: VideoInventoryCandidate[]
  manual?: VideoInventoryCandidate[]
}) {
  const combined = [
    ...input.video,
    ...(input.transcript || []),
