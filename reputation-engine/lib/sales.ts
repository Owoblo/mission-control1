...
const LOCKED_ESTIMATE_LINE_ITEM_DESCRIPTIONS = new Set([
  'Moving Services — Agreed Rate',
  'Long-Distance Moving Service — All Inclusive',
])

export function hasLockedEstimateLineItem(lineItems: QuoteLineItem[]) {
  return lineItems.some(item => LOCKED_ESTIMATE_LINE_ITEM_DESCRIPTIONS.has((item.description || '').trim()))
}

function quoteLineItemsEqual(left: QuoteLineItem[], right: QuoteLineItem[]) {
  return left.length === right.length && left.every((item, index) => {
    const other = right[index]
    return (
      item.description === other.description &&
      (item.details || '') === (other.details || '') &&
      Number(item.amount || 0) === Number(other.amount || 0)
    )
  })
}

/**
 * Replace calculated estimate rows without disturbing rep-entered rows.
 *
 * Returning `current` when the result is unchanged is important: quote pages
 * recalculate in an effect, and a fresh-but-identical array would otherwise
 * create a render/effect feedback loop that can starve client navigation.
 */
export function reconcileEstimatedQuoteLineItems(
  current: QuoteLineItem[],
  calculated: QuoteLineItem[]
) {
  if (hasLockedEstimateLineItem(current)) return current
  const manualItems = current.filter(item =>
    item.pricingSource === 'manual' || !isAutoGeneratedEstimateLineItemDescription(item.description)
  )
  const manuallyPricedDescriptions = new Set(manualItems.map(item => (item.description || '').trim()))
  const next = [
    ...calculated.filter(item => !manuallyPricedDescriptions.has((item.description || '').trim())),
    ...manualItems,
  ]
  return quoteLineItemsEqual(current, next) ? current : next
}
