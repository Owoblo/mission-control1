...
export interface DynamicPricingInput {
  ...
  branchCapacityPct?: number
  scopeConfidence?: 'high' | 'medium' | 'low'
  routeRisk?: PricingRisk
  accessRisk?: PricingRisk
  complexity?: 'standard' | 'multi_stop' | 'storage_staged' | 'conjoint' | 'junk_addon'
  referralDiscountPct?: number
}

export interface DynamicPricingAdvice {
 ...
 */
export function adviseDynamicPrice(input: DynamicPricingInput): DynamicPricingAdvice {
  ...
  const riskAdjustment = { low: 0, medium: 0.03, high: 0.07 } as const
  adjustment += riskAdjustment[input.routeRisk || 'low']
  adjustment += riskAdjustment[input.accessRisk || 'low']
  if (input.routeRisk === 'high') reasons.push('Route has elevated operational uncertainty.')
  if (input.accessRisk === 'high') reasons.push('Access has elevated labor or delay risk.')
