...
export async function estimateRouteContext(input: {
  ...
  // A location outside an established branch is an open sales market. Anchor
  // the estimate at the customer's origin instead of inventing an Ontario
  // deadhead. Staff can still explicitly select a branch when that is the real
  // operating model for an intercity job.
