...
export function evaluateQuoteReadiness(lead: CRMLead, quote?: Pick<CRMQuote, 'billingModel' | 'quoteType' | 'originAddress' | 'destAddress'>) {
  ...
  if (factors.destFloors === undefined || factors.destHasElevator === undefined || factors.destParkingOk === undefined) blockers.push('Destination stairs, elevator, and truck access are incomplete.')
  ...
  if (factors.destHasElevator && factors.destElevatorReserved !== true) blockers.push('Destination elevator reservation is not confirmed.')
  if (!(quote?.originAddress || lead.originAddress)?.trim() || !(quote?.destAddress || lead.destAddress)?.trim()) blockers.push('Both move addresses are required.')
