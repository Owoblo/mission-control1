import type { JobFactors, PricingBreakdown, QuoteLeg, QuoteLineItem } from './types'
...
export interface ServiceWorkPackage {
  id: string
  ...
  scope: string[]
  evidence: 'system' | 'rep_entered'
  needsReview: boolean
  reviewReason?: string
}

export interface ServiceProfitabilityPlan {
  packages: ServiceWorkPackage[]
  revenue: number
  directCost: number
  grossProfit: number
  grossMarginPct: number
...
export function classifyServiceLine(description: string): ServiceCategory {
  const value = description.toLowerCase()
  if (/\b(?:pack|packs|packed|packing|unpack|unpacks|unpacked|unpacking)\b/.test(value)) return 'packing'
  if (/\b(box|boxes|tape|paper|material|supply|supplies)\b/.test(value)) return 'materials'
  if (/\b(storage|locker|container)\b/.test(value)) return 'storage'
  if (/\b(junk|disposal|dump|remove)\b/.test(value)) return 'junk'
  if (/\b(clean|cleaning)\b/.test(value)) return 'cleaning'
  if (/\b(piano|safe|pool table|hot tub|specialty|mount|dismount)\b/.test(value)) return 'specialty'
  if (/\b(move|moving|delivery|labou?r|truck|crew)\b/.test(value)) return 'moving'
  return 'custom'
}

function packageFromLine(item: QuoteLineItem, index: number): ServiceWorkPackage {
  const category = classifyServiceLine(item.description || '')
  const revenue = currency(Number(item.amount || 0))
  const scope = [item.details?.trim()].filter(Boolean) as string[]
...
export function buildServiceProfitabilityPlan(input: {
  lineItems: QuoteLineItem[]
  legs?: QuoteLeg[]
  jobFactors?: JobFactors
  pricingBreakdown?: PricingBreakdown | null
}): ServiceProfitabilityPlan {
  ...
  const costDriver: Record<ServiceCategory, number> = {
    moving: 1,
    packing: 0.8,
    storage: 0.45,
    junk: 0.75,
  ...
  const status =
    revenue <= 0 || grossMarginPct < 45 || protections.length >= 3
      ? 'blocked'
      : grossMarginPct < 55 || protections.length > 0 || hasUnpricedService
        ? 'watch'
        : 'healthy'

  if (grossMarginPct < 45 && revenue > 0) protections.push('Projected gross margin is below the 45% protection floor.')
  if (hasUnpricedService) protections.push('At least one service needs a price or an explicit complimentary/included label.')
