...
export function buildPaymentRecord(input: {
  ...
  cardLast4?: string
  cardBrand?: string
  cardFunding?: PaymentRecord['cardFunding']
  methodLabel?: string
  recordedBy?: string
  ...
  return {
    ...
    publicToken: crypto.randomUUID(), kind: input.kind, method: input.method, methodLabel: input.methodLabel || PAYMENT_METHOD_LABELS[input.method],
    ...
    cardLast4: input.cardLast4, cardBrand: input.cardBrand, cardFunding: input.cardFunding,
    recordedBy: input.recordedBy, recordedByUserId: input.recordedByUserId,
  }
}
