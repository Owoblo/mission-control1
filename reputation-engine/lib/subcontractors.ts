...
export function evaluateSubcontractorEligibility(
  ...
  if (requirements.moveDate && contractor.insuranceExpiresAt && contractor.insuranceExpiresAt < requirements.moveDate) reasons.push('Insurance expires before move date')
  if (contractor.insuranceExpiresAt && !requirements.moveDate) warnings.push(`Insurance expires ${contractor.insuranceExpiresAt}`)

  const totalOutcomes = contractor.completedJobs + contractor.cancelledJobs
  const completionRate = totalOutcomes ? contractor.completedJobs / totalOutcomes : 0.75
  const score = Math.max(0, Math.min(100, Math.round(
    45 + completionRate * 30 + (contractor.averageRating || 4) * 5 - reasons.length * 30
  )))
...
export function buildSubcontractorOfferSms(input: {
  ...
  const hours = input.estimatedHoursMin || input.estimatedHoursMax
