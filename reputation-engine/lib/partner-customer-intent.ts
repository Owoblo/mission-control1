export function isPartnerMovingLeadIntent(value?: string | null) {
  const text = String(value || '').toLowerCase().replace(/\s+/g, ' ').trim()
  if (!text) return false

  const genericReferralOnly =
    /\b(if|when|whenever)\b.{0,40}\b(my|our|any)\s+(clients?|customers?)\b.{0,45}\b(move|moving|movers?)\b/.test(text) &&
    !/\b(i|i'm|i am|we|we're|we are|my wife|my husband|my family)\b.{0,45}\b(move|moving|relocat)/.test(text)
  if (genericReferralOnly) return false

  return (
    /\bappointment booked\b.{0,45}\bmoving service\b/.test(text) ||
