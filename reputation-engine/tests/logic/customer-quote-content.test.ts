...
import { buildCustomerCarePlan, buildCustomerQuoteScope, sanitizeCustomerQuoteText } from '../../lib/customer-quote-content'
...
test('removes internal margin and manager-review language from public quote copy', () => {
  ...
  assert.equal(
    sanitizeCustomerQuoteText(input),
    'Provisional estimate. Final pricing will be confirmed once we verify: Packing status is not confirmed.',
  )
})
