drop function if exists public.list_sms_thread_summaries();

create function public.list_sms_thread_summaries(
...
as $$
  with raw_messages as (
    select
      ...
      body, created_at, direction, lead_id, twilio_sid
    ...
    select
      ...
      body, created_at, direction, lead_id
    from raw_messages where length(contact_digits) >= 7
  ), summaries as (
    select
      contact_phone,
    ...
    from normalized group by contact_phone
  )
  select * from summaries
  where nullif(trim(p_search), '') is null
     or contact_phone ilike '%' || trim(p_search) || '%'
     or last_message ilike '%' || trim(p_search) || '%'
  order by last_at desc
  limit least(greatest(p_limit, 1), 250)
  offset greatest(p_offset, 0);
$$;

revoke all on function public.list_sms_thread_summaries(integer, integer, text) from public, anon, authenticated;
grant execute on function public.list_sms_thread_summaries(integer, integer, text) to service_role;
