...
exports.deriveMoveLogisticsPlan = deriveMoveLogisticsPlan;
...
function formatHourAsTime(hourValue) {
...
function formatClock(startHour, offsetHours) {
...
function deriveMoveLogisticsPlan(input) {
    ...
    let recommendation = 'one_truck_sequence';
    if (missingRouteCount > 0)
        recommendation = 'needs_route_data';
    else if (totalHours > maxSameDayHours)
        recommendation = totalHours >= 12 ? 'split_day' : 'two_truck_parallel';
    else if (truckCount > 1)
        recommendation = totalHours + 1.5 <= maxSameDayHours ? 'one_truck_shuttle' : 'two_truck_parallel';
    ...
    if (constraintFit.status === 'runs_late')
        recommendation = pickupContexts.length >= 2 ? 'two_truck_parallel' : 'split_day';
    const label = recommendation === 'needs_route_data'
        ...
        : recommendation === 'split_day'
            ...
            : recommendation === 'one_truck_shuttle'
                ...
                : recommendation === 'two_truck_parallel'
                    ...
                    : 'One-truck sequence';
    const riskNotes = [];
    if (capacityUsedPct >= 95)
        riskNotes.push('Single truck is near capacity; confirm hidden inventory and boxes.');
    if (truckCount > 1)
        riskNotes.push(`Combined volume needs ${truckCount} trucks at ${singleTruckCapacity.toLocaleString()} cu ft capacity each.`);
    if (totalHours > maxSameDayHours)
        riskNotes.push(`Projected ${totalHours}h day may finish late; discuss a split-day plan.`);
    if (missingRouteCount > 0)
        riskNotes.push(`${missingRouteCount} leg${missingRouteCount === 1 ? '' : 's'} still need route calculation.`);
    if (pendingContextCount > 0)
        riskNotes.push(`${pendingContextCount} pickup context${pendingContextCount === 1 ? '' : 's'} still need inventory confirmation; pricing and margin are provisional.`);
    if (constraintFit.status === 'adjust_start' || constraintFit.status === 'runs_late')
        riskNotes.push(constraintFit.note);
    ...
    const parallelHours = roundQuarterHour(Math.max(loadAHours + firstDrive + unloadAHours, loadBHours + secondDrive + unloadBHours) + Math.max(0.5, Math.min(firstDrive + secondDrive, 1)));
