...
exports.OPERATING_STAGE_META = void 0;
exports.deriveOperatingStage = deriveOperatingStage;
exports.deriveJobReadiness = deriveJobReadiness;
exports.deriveOperatingExceptions = deriveOperatingExceptions;
const operations_1 = require("./operations");
const move_relationship_1 = require("./move-relationship");
exports.OPERATING_STAGE_META = {
...
function requiresTruck(lead, quote) {
    return Boolean((0, operations_1.getQuotedTruckCount)(lead, quote));
}
function moveDate(lead, quote) {
...
function deriveOperatingStage(lead, quote) {
    if (lead.stage === 'lost')
        return 'closed';
    if (lead.reviewCompletedAt)
        return 'reviewed';
    if (lead.stage === 'completed' || lead.stage === 'customer_success') {
        return isPaid(lead, quote) ? 'paid' : 'completed';
    }
    if (executionTimestamp(lead, 'return_yard') || executionTimestamp(lead, 'unload_complete'))
        return 'completed';
    if (executionTimestamp(lead, 'arrive_origin') || executionTimestamp(lead, 'crew_depart_yard'))
        return 'in_progress';
    if (hasConfirmedCrew(lead))
        return 'dispatched';
    if (lead.stage === 'booked') {
        const readiness = deriveJobReadiness(lead, quote);
        if (readiness.status === 'fully_ready')
            return 'prepared';
        if (hasDeposit(lead, quote))
            return 'confirmed';
        return 'booked';
    }
    if (quote?.sentAt || lead.stage === 'quoted' || lead.stage === 'tentative')
        return 'quote';
    if (lead.stage === 'pricing' || lead.stage === 'estimate_completed')
        return 'estimate';
    if (lead.stage === 'estimate_scheduled')
        return 'estimate';
    if (lead.stage === 'contacted' || lead.stage === 'nurture')
        return 'qualified';
    return 'lead';
}
function dimension(key, label, checks) {
...
function deriveJobReadiness(lead, quote) {
    const checklist = (0, operations_1.deriveOpsChecklist)(lead);
    ...
    const dimensions = [
        ...
        dimension('equipment', 'Equipment', [
            [!truckNeeded || (0, operations_1.isTruckReservationComplete)(lead.truckReservationStatus), 'Truck not reserved'],
...
function deriveOperatingExceptions(lead, quote) {
    const items = [];
    const href = `/sales/leads/${lead.id}`;
    const add = (key, exception) => items.push({
        id: `${lead.id}:${key}`,
        leadId: lead.id,
        customer: lead.name,
        branch: lead.branch,
    ...
    for (const promise of (lead.promises || []).filter(item => item.status === 'open')) {
        ...
        if (Number.isFinite(due) && due <= now + 24 * 60 * 60 * 1000)
            add(`promise:${promise.id}`, {
    ...
    if (activeSales && !lead.assignedRepUserId && !lead.assignedRepName && !lead.assignedRep)
        add('owner', { severity: 'urgent', environment: 'Intake', title: 'No owner assigned', detail: 'Incoming demand has no accountable person.', action: 'Assign an owner' });
    if (activeSales && ['estimate_completed', 'pricing'].includes(lead.stage) && !quote?.sentAt)
        add('quote', { severity: 'attention', environment: 'Sales', title: 'Quote has not been sent', detail: 'The estimate is far enough along to require a customer decision.', action: 'Finish and send quote' });
    if (activeSales && lead.lastInboundAt && (!lead.lastHumanOutboundAt || lead.lastInboundAt > lead.lastHumanOutboundAt))
        add('reply', { severity: 'urgent', environment: 'Sales', title: 'Customer is waiting', detail: 'The latest inbound message has no later human response.', action: 'Respond now' });
    if (lead.stage === 'booked') {
        const readiness = deriveJobReadiness(lead, quote);
        const days = daysFromToday(moveDate(lead, quote));
        if (!hasDeposit(lead, quote))
            add('deposit', { severity: days !== null && days <= 2 ? 'urgent' : 'attention', environment: 'Operations', title: 'Booked without deposit', detail: 'Financial confirmation is still incomplete.', action: 'Resolve deposit' });
        for (const item of readiness.dimensions.flatMap(item => item.missing).slice(0, 4))
            add(`readiness:${item}`, { severity: days !== null && days <= 1 ? 'urgent' : 'attention', environment: 'Operations', title: item, detail: `${readiness.label} · ${readiness.percent}% complete`, action: 'Complete job preparation' });
    }
    const openIssues = lead.moveExecutionLog?.issues?.filter(issue => issue.severity === 'high' || issue.severity === 'medium') || [];
    for (const issue of openIssues)
        add(`issue:${issue.id}`, { severity: issue.severity === 'high' ? 'urgent' : 'attention', environment: 'Live execution', title: `${issue.category.replaceAll('_', ' ')} issue`, detail: issue.note, action: 'Review live issue' });
    if ((lead.stage === 'completed' || lead.stage === 'customer_success') && !isPaid(lead, quote))
        add('balance', { severity: 'urgent', environment: 'Completion & care', title: 'Completed but unpaid', detail: 'The job is complete and a balance remains open.', action: 'Resolve final payment' });
    if ((lead.stage === 'completed' || lead.stage === 'customer_success') && !lead.reviewSentAt)
        add('review', { severity: 'attention', environment: 'Completion & care', title: 'Care follow-up not sent', detail: 'The completed job has no recorded review request.', action: 'Complete customer follow-up' });
    if (lead.stage === 'completed' || lead.stage === 'customer_success') {
        const lifecycleGaps = (0, move_relationship_1.moveRelationshipLifecycleGaps)({
            context: lead.opportunityContext,
            signals: lead.attributionSignals,
        });
        if (lifecycleGaps.length)
            add('relationship-lifecycle', {
                severity: 'attention',
