...
exports.UHAUL_RATE_PER_KM = exports.SALES_TIME_ZONE = exports.QUOTE_STATUSES = exports.FOLLOW_UP_STATUSES = exports.PROPERTY_TYPE_OPTIONS = exports.PROPERTY_BEDROOM_OPTIONS = exports.CRM_LEAD_SOURCES = exports.SALES_BRANCHES = exports.DEPOSIT_METHODS = exports.LEAD_CONTEXT_FLAGS = exports.LOST_REASONS = exports.CLOSED_LEAD_STAGES = exports.BOOKED_LIKE_STAGES = exports.SALES_LEAD_STAGES = void 0;
exports.isBookedLikeStage = isBookedLikeStage;
exports.isClosedLeadStage = isClosedLeadStage;
exports.getSalesBranchLabel = getSalesBranchLabel;
exports.detectSalesBranchFromLocation = detectSalesBranchFromLocation;
exports.isLocationWithinBranchServiceArea = isLocationWithinBranchServiceArea;
exports.uid = uid;
exports.formatMoney = formatMoney;
exports.dateStamp = dateStamp;
exports.formatDate = formatDate;
exports.formatDateTime = formatDateTime;
exports.genQuoteNumber = genQuoteNumber;
exports.validUntil = validUntil;
exports.calculateLeadScore = calculateLeadScore;
exports.getLeadAssignedRepName = getLeadAssignedRepName;
exports.getLeadAssignedRepKey = getLeadAssignedRepKey;
exports.normalizeLeadSource = normalizeLeadSource;
exports.getLeadSourceLabel = getLeadSourceLabel;
exports.deriveLeadFollowUpStatus = deriveLeadFollowUpStatus;
exports.normalizeLead = normalizeLead;
exports.normalizeClient = normalizeClient;
exports.normalizeQuote = normalizeQuote;
exports.isAutoGeneratedEstimateLineItemDescription = isAutoGeneratedEstimateLineItemDescription;
exports.hasLockedEstimateLineItem = hasLockedEstimateLineItem;
exports.reconcileEstimatedQuoteLineItems = reconcileEstimatedQuoteLineItems;
exports.getCrewRate = getCrewRate;
exports.getDefaultDepositRate = getDefaultDepositRate;
exports.getDefaultPaymentTerms = getDefaultPaymentTerms;
exports.isInvoiceStylePaymentTerms = isInvoiceStylePaymentTerms;
exports.paymentTermsLabel = paymentTermsLabel;
exports.computeCommercialCostLayer = computeCommercialCostLayer;
exports.suggestCrewSize = suggestCrewSize;
exports.needsDisassembly = needsDisassembly;
exports.suggestDisassemblyCount = suggestDisassemblyCount;
exports.suggestTruckCount = suggestTruckCount;
exports.computeJobPenalties = computeJobPenalties;
exports.normalizeFollowUp = normalizeFollowUp;
exports.syncLeadFromQuoteStatus = syncLeadFromQuoteStatus;
exports.estimateLeadQuote = estimateLeadQuote;
exports.computeQuoteTotals = computeQuoteTotals;
exports.deriveInventoryMetrics = deriveInventoryMetrics;
exports.buildSalesSummary = buildSalesSummary;
exports.getClientMap = getClientMap;
exports.getQuoteMap = getQuoteMap;
const sales_phones_1 = require("./sales-phones");
const move_policy_1 = require("./move-policy");
const operations_1 = require("./operations");
const packing_materials_1 = require("./packing-materials");
const realtor_opportunity_1 = require("./realtor-opportunity");
const sales_automation_qualification_1 = require("./sales-automation-qualification");
function normalizeOptionalText(value) {
    const trimmed = value?.trim();
    return trimmed ? trimmed : undefined;
}
exports.SALES_LEAD_STAGES = [
...
exports.BOOKED_LIKE_STAGES = ['booked', 'completed', 'customer_success'];
exports.CLOSED_LEAD_STAGES = [...exports.BOOKED_LIKE_STAGES, 'lost'];
function isBookedLikeStage(stage) {
    return !!stage && exports.BOOKED_LIKE_STAGES.includes(stage);
}
function isClosedLeadStage(stage) {
    return !!stage && exports.CLOSED_LEAD_STAGES.includes(stage);
}
exports.LOST_REASONS = [
...
exports.DEPOSIT_METHODS = ['E-Transfer', 'Credit Card', 'Cash', 'Cheque', 'Other'];
exports.SALES_BRANCHES = [
    { id: 'windsor', label: 'Windsor' },
    { id: 'waterloo', label: 'Waterloo / KW' },
    { id: 'london', label: 'London' },
    { id: 'ottawa', label: 'Ottawa' },
];
exports.CRM_LEAD_SOURCES = [
...
exports.PROPERTY_BEDROOM_OPTIONS = [
...
function normalizeLocationText(...values) {
    return values
        .filter(Boolean)
        .join(' ')
        .normalize('NFD')
...
function getSalesBranchLabel(branch) {
    return exports.SALES_BRANCHES.find(item => item.id === branch)?.label || 'Unassigned';
}
function detectSalesBranchFromLocation(...values) {
    const normalized = normalizeLocationText(...values);
    if (!normalized)
        return undefined;
    const paddedLocation = ` ${normalized} `;
    for (const [branch, aliases] of Object.entries(SALES_BRANCH_AREAS)) {
...
function isLocationWithinBranchServiceArea(branch, ...values) {
    if (!branch)
        return false;
    const normalized = normalizeLocationText(...values);
    if (!normalized)
...
exports.SALES_TIME_ZONE = 'America/Toronto';
function dateStamp(value = new Date(), timeZone = exports.SALES_TIME_ZONE) {
...
function genQuoteNumber(clientName) {
    const now = new Date();
...
function calculateLeadScore(lead) {
    let score = 0;
    if (lead.source === 'customer_referral' || lead.source === 'partner_referral' || lead.source === 'referral')
        score += 25;
    else if (lead.source === 'google_online_search' || lead.source === 'google')
        score += 20;
    else if (lead.source === 'direct_mail')
        score += 15;
    else if (lead.source === 'repeat_customer' || lead.source === 'repeat')
        score += 30;
    else if (lead.source === 'destination_opportunity')
        score += 12;
    if (lead.stage === 'quoted')
        score += 20;
    else if (lead.stage === 'customer_success')
        score += 50;
    else if (lead.stage === 'completed')
        score += 45;
    else if (lead.stage === 'booked')
        score += 40;
    else if (lead.stage === 'pricing')
        score += 15;
    else if (lead.stage === 'nurture')
        score += 8;
    else if (lead.stage === 'contacted')
        score += 10;
    if ((lead.inventory || []).length > 0)
        score += 10;
    if (lead.phone)
        score += 5;
    if (lead.quoteId)
        score += 15;
    if (lead.leadKind === 'realtor_opportunity')
        score += 5;
...
function getLeadSourceLabel(source) {
    ...
    if (!normalized)
        return 'Other';
    return exports.CRM_LEAD_SOURCES.find(item => item.id === normalized)?.label || normalized.replace(/_/g, ' ');
}
function deriveLeadFollowUpStatus(lead) {
    if (lead.followUpStatus)
        return lead.followUpStatus;
    if (lead.stage === 'lost' || lead.stage === 'completed' || lead.stage === 'customer_success') {
        return 'followed_up';
    }
    const today = new Date().toISOString().slice(0, 10);
    const hasHumanFollowUp = !!lead.lastHumanOutboundAt &&
    ...
    if (lead.followUpDate) {
        ...
        return hasHumanFollowUp ? 'following_up' : 'pending';
    }
    if (hasHumanFollowUp) {
        return 'following_up';
    }
...
function normalizeLead(lead) {
    const leadKind = lead.leadKind || 'customer';
    ...
    const inventory = (0, move_policy_1.applyMovePolicyToInventory)(Array.isArray(lead.inventory) ? lead.inventory : [], {
    ...
    const normalizedLead = {
        ...
        identityPhone: normalizeOptionalText(lead.identityPhone) || (0, sales_phones_1.normalizePhone)(normalizedPhone) || undefined,
        ...
        crewPayouts: (0, operations_1.normalizeCrewPayouts)(lead.crewPayouts),
    ...
    if (normalizedLead.qualificationState) {
        normalizedLead.qualificationState = (0, sales_automation_qualification_1.buildLeadQualificationState)(normalizedLead, {
    ...
    if (normalizedLead.leadKind === 'realtor_opportunity' && normalizedLead.primaryContactRole === 'realtor') {
        return (0, realtor_opportunity_1.applyRealtorContactToOpportunityLead)(normalizedLead, {
...
const LOCAL_CREW_RATES_TRUCK_AWARE = {
    '1-1': 100,
    '2-1': 160,
    '3-1': 200,
...
function isAutoGeneratedEstimateLineItemDescription(description) {
    const normalized = (description || '').trim();
    if (!normalized)
        return false;
    if (normalized.startsWith('[Leg '))
        return true;
...
function hasLockedEstimateLineItem(lineItems) {
    return lineItems.some(item => LOCKED_ESTIMATE_LINE_ITEM_DESCRIPTIONS.has((item.description || '').trim()));
}
function quoteLineItemsEqual(left, right) {
    return left.length === right.length && left.every((item, index) => {
        const other = right[index];
        return (item.description === other.description &&
 ...
 */
function reconcileEstimatedQuoteLineItems(current, calculated) {
    if (hasLockedEstimateLineItem(current))
        return current;
...
function getCrewRate(crewSize, moveType, truckCount) {
    ...
    const rateTable = moveType === 'labor-only'
        ? LABOR_ONLY_CREW_RATES
        : moveType === 'packing'
            ? PACKING_CREW_RATES
            : LOCAL_CREW_RATES;
    if (rateTable[normalizedCrew])
        return rateTable[normalizedCrew];
    if (normalizedCrew > 4) {
        return (rateTable[4] || rateTable[3]) + (normalizedCrew - 4) * 60;
    }
    return rateTable[3];
}
function getDefaultDepositRate(moveType) {
    if (moveType === 'commercial')
        return 0;
    return moveType === 'long-distance' ? 0.4 : 0.2;
}
function getDefaultPaymentTerms(moveType) {
    return moveType === 'commercial' ? 'approval_invoice' : 'deposit_required';
}
function isInvoiceStylePaymentTerms(paymentTerms) {
...
function paymentTermsLabel(paymentTerms) {
    if (paymentTerms === 'approval_invoice')
        return 'Approval + invoice';
    if (paymentTerms === 'invoice_net_7')
        return 'Invoice Net 7';
    if (paymentTerms === 'invoice_net_15')
        return 'Invoice Net 15';
    if (paymentTerms === 'invoice_net_30')
        return 'Invoice Net 30';
    if (paymentTerms === 'po_required')
        return 'PO required';
    return 'Deposit required';
}
function computeCommercialCostLayer(factors, revenue = 0) {
...
function addressLooksLikeHouse(address, propertyType) {
    if (propertyType === 'detached_house' || propertyType === 'townhouse')
        return true;
...
function accessTextSaysLimited(...values) {
...
function normalizeHouseAccessAssumptions(lead, factors) {
    if (!factors)
        return factors;
    const next = { ...factors };
    const limitedAccessNotes = accessTextSaysLimited(lead.originAccess, lead.destAccess, lead.parkingNotes, lead.notes);
    if (addressLooksLikeHouse(lead.originAddress, lead.propertyType) &&
        (next.originFloors || 1) <= 2 &&
        !limitedAccessNotes) {
        if (next.originParkingOk === false)
    ...
    if (addressLooksLikeHouse(lead.destAddress, lead.propertyType) &&
        (next.destFloors || 1) <= 2 &&
        !limitedAccessNotes) {
        if (next.destParkingOk === false)
...
function suggestCrewSize(totalWeightLbs, totalCubicFeet, includedInventory) {
    const heavyKeywords = [
        'sectional',
        'piano',
        'safe',
    ...
    if (totalWeightLbs >= 6500 || totalCubicFeet >= 1400 || oversizedCount >= 10)
        return 4;
    if (totalWeightLbs >= 2200 || totalCubicFeet >= 550 || oversizedCount >= 4)
        return 3;
    if (totalWeightLbs >= 700 || totalCubicFeet >= 180)
        return 2;
    return 1;
}
function estimatePackingDayPlan(input) {
    const includedInventory = input.inventory.filter(item => item.included !== false);
    const uniqueRooms = new Set(includedInventory
        .map(item => (item.room || '').trim())
        .filter(Boolean)).size;
    const estimatedBoxes = Math.max(0, Number(input.estimatedBoxes || 0) ||
    ...
    const packingCrewSize = input.crewSizeHint >= 4 || input.totalCubicFeet >= 1100 || estimatedBoxes >= 65
        ? 3
        : 2;
    ...
    const roomHours = uniqueRooms > 0
        ...
        : 0;
    const packingHours = Math.max(packingCrewSize === 3 ? 5 : 4, roundQuarterHour(Math.max(volumeHours, boxHours, roomHours)));
...
exports.UHAUL_RATE_PER_KM = 1; // quick rental estimate used for long-distance quoting
...
function needsDisassembly(name) {
    const lower = name.toLowerCase();
    if (DISASSEMBLY_EXCLUSIONS.some(ex => lower.includes(ex)))
        return false;
    return DISASSEMBLY_KEYWORDS.some(kw => lower.includes(kw));
}
function getTruckRateMultiplier(truckCount) {
    if (truckCount >= 3)
        return THREE_TRUCK_RATE_MULTIPLIER;
    if (truckCount === 2)
        return EXTRA_TRUCK_RATE_MULTIPLIER;
    return 1;
}
function estimateRequiredTrucks(totalCubicFeet, totalWeightLbs) {
...
function suggestDisassemblyCount(inventory) {
...
function suggestTruckCount(totalCubicFeet, totalWeightLbs = 0, moveType) {
...
function computeJobPenalties(factors) {
    const penalties = [];
    // Origin access
    ...
    const extraCubicFeet = (factors.garageCubicFeet || 0) +
...
function normalizeFollowUp(log) {
...
function syncLeadFromQuoteStatus(lead, quote) {
    const nextStage = quote.status === 'accepted' || quote.status === 'invoiced'
        ? 'booked'
    ...
    return normalizeLead({
        ...
        quoteId: quote.id,
        stage: nextStage,
        tentativeReservationStatus: lead.tentativeReservationStatus === 'active' && nextStage === 'booked'
            ? 'converted'
...
function estimateSingleLeadQuote(lead, overrides, factors) {
    ...
    const specialtyItemFlags = [];
    if (factors?.hasPiano ?? lead.jobFactors?.hasPiano)
        specialtyItemFlags.push('Upright Piano');
    if (factors?.hasSafe ?? lead.jobFactors?.hasSafe)
        specialtyItemFlags.push('Heavy Safe');
    const rawFactors = factors ?? lead.jobFactors;
    const activeFactors = normalizeHouseAccessAssumptions(lead, rawFactors
        ? {
            ...rawFactors,
    ...
    if (lead.moveType === 'commercial' && activeFactors) {
        ...
        commercialAdjustments.forEach(adjustment => {
            ...
            if (!adjustment.isFlagOnly)
                extraHours += adjustment.hours;
        });
    }
    ...
    const originToDestHours = roundQuarterHour(routeContext?.originToDestinationHours ?? overrides?.driveHours ?? (isLongDistance ? 1.5 : 0.75));
    const yardToOriginHours = roundQuarterHour(isLaborOnly ? 0 : routeContext?.yardToOriginHours ?? 0);
    const returnTripHours = roundQuarterHour(routeContext?.returnTripHours ??
        (routeCategory === 'long-distance' && !missingDestination ? originToDestHours : 0));
    const billableDriveHours = roundQuarterHour(routeContext?.billableDriveHours ??
        (missingDestination
            ? 0
    ...
    const operationalDriveHours = roundQuarterHour(routeContext?.operationalDriveHours ??
        (missingDestination
            ? 0
            : routeCategory === 'long-distance'
                ? yardToOriginHours + originToDestHours + returnTripHours
    ...
    const autoUhaulChargeEstimate = isLongDistance && operationalDistanceKm
        ? roundCurrency(Math.max(0, operationalDistanceKm) * truckCount * exports.UHAUL_RATE_PER_KM)
        : 0;
    const longDistanceTruckCost = Number(overrides?.longDistanceTruckCost || 0) || autoUhaulChargeEstimate;
    const forcedSingleTruckTwoTrips = !missingDestination &&
        !isLongDistance &&
        !isPacking &&
        !isLaborOnly &&
        truckCount === 1 &&
    ...
    const additionalTripDistanceKm = forcedSingleTruckTwoTrips
        ...
        : 0;
    const loadHours = totalWeightLbs > 0
    ...
    const unloadHours = totalWeightLbs > 0
    ...
    const tripStrategy = truckCount >= 3 ? 'three_trucks' : forcedSingleTruckTwoTrips ? 'single_truck_two_trips' : truckCount === 2 ? 'two_trucks' : 'single_truck';
    let twoTripComparison = null;
    let multiTruckOption = null;
    if (!missingDestination && !isLongDistance && !isPacking && !isLaborOnly && (truckCount >= 2 || totalCubicFeet >= TWO_TRIP_ZONE_CF)) {
        ...
        const secondTripHandlingHours = roundQuarterHour(threeMoverLoadHours * trip2Frac * 0.55 + threeMoverUnloadHours * trip2Frac * 0.55 * 0.85);
    ...
    const packingMaterialsEstimate = isPacking || activeFactors?.packingStatus === 'not-started' || activeFactors?.packingStatus === 'partial'
        ? (0, packing_materials_1.buildPackingMaterialsEstimate)({
    ...
    const adjustmentBreakdown = adjustmentCategories
        ...
        label: entry.label,
        hours: roundQuarterHour(penalties
    ...
    const computedRevenue = roundCurrency(lead.moveType === 'commercial'
    ...
    const phaseDetail = totalWeightLbs > 0
    ...
    const pricingBreakdown = {
        ...
        uhaulRatePerKm: isLongDistance ? exports.UHAUL_RATE_PER_KM : undefined,
...
function clampInventorySharePct(value, fallback) {
...
function buildScaledLeadSnapshot(lead, totalCubicFeet, totalWeightLbs, sharePct) {
...
function buildLegRouteContext(leg, fallbackCategory) {
    const deliveryLike = leg.type === 'delivery' || leg.type === 'junk';
    const pricingStatus = leg.pricingStatus ||
    ...
    const billableDriveHours = roundQuarterHour(deliveryLike
        ? Number(leg.driveHours || leg.billableDriveHours || 0)
        : Number(leg.billableDriveHours || leg.driveHours || 0));
    const operationalDriveHours = roundQuarterHour(deliveryLike
    ...
    const billableDistanceKm = roundCurrency(deliveryLike
        ? Number(leg.distanceKm || leg.billableDistanceKm || 0)
        : Number(leg.billableDistanceKm || leg.distanceKm || 0));
    const operationalDistanceKm = roundCurrency(deliveryLike
...
function buildLegFactors(baseFactors, leg) {
    ...
    if (leg.type === 'storage') {
        ...
        if (next.disassemblyItemCount)
            apply('disassemblyMode', 'disassemble_only');
    }
    if (leg.type === 'storage_delivery' || leg.type === 'delivery') {
        ...
        if (next.disassemblyItemCount)
            apply('disassemblyMode', 'reassemble_only');
    }
    return touched ? next : undefined;
}
function buildConjointLegFactors(baseFactors, legIndex) {
    if (!baseFactors)
        return undefined;
    const next = { ...baseFactors, conjointMove: false };
    if (legIndex === 0) {
        next.destFloors = 1;
        next.destHasElevator = false;
        next.destElevatorReserved = undefined;
        next.destParkingOk = true;
        if (next.disassemblyItemCount)
            next.disassemblyMode = 'disassemble_only';
        return next;
    }
...
function getLegServiceLabel(leg) {
    if (leg.type === 'storage')
        return 'House → Storage';
    if (leg.type === 'storage_delivery')
        return 'Storage → New Home';
    if (leg.type === 'delivery')
        return 'Additional Delivery Stop';
    if (leg.type === 'junk')
        return 'Junk Removal Stop';
    return 'Moving Service';
}
function getLegDisplayLabel(leg, index) {
...
function buildMultiLegEstimate(lead, overrides, factors) {
    const legs = (overrides?.legs || []).filter(leg => !!(leg.originAddress || leg.originCity || leg.destAddress || leg.destCity || leg.label || leg.type));
    ...
    for (let index = 0; index < legs.length; index += 1) {
        ...
        if (leg.type === 'delivery') {
            ...
            const deliveryEstimate = estimateSingleLeadQuote(deliveryScaledLead, {
                crewSize: baseEstimate.crewSize,
                truckCount: baseEstimate.truckCount,
            ...
            const deliveryDriveBufferHours = roundQuarterHour(deliveryRouteContext.routeCategory === 'long-distance' ? 0 : deliveryDriveHours * 0.1);
            const deliveryLoadUnloadBufferHours = roundQuarterHour(deliveryRouteContext.routeCategory === 'long-distance' ? 0 : (deliveryUnloadHours + deliveryPenaltyHours) * 0.1);
            const deliveryTotalHours = Math.max(1.5, roundQuarterHour(deliveryUnloadHours +
                deliveryDriveHours +
                deliveryPenaltyHours +
                deliveryDriveBufferHours +
                deliveryLoadUnloadBufferHours));
            const deliveryOperationalHours = roundQuarterHour(deliveryUnloadHours +
                deliveryOperationalDriveHours +
                deliveryPenaltyHours +
                deliveryDriveBufferHours +
                deliveryLoadUnloadBufferHours);
            const deliveryLongDistanceOperationalBase = Number(deliveryEstimate.longDistanceTruckCost || 0) +
            ...
            const deliveryLongDistanceMarkupAmount = deliveryRouteContext.routeCategory === 'long-distance'
                ...
                : 0;
            const deliveryAmount = roundCurrency((deliveryTotalHours * baseEstimate.pricingBreakdown.crewRatePerHour) +
            ...
            collectedPenalties.push(...deliveryEstimate.pricingBreakdown.penalties.map(penalty => ({
                ...penalty,
        ...
        if (baseFactors?.conjointMove && legs.length >= 2 && index < 2 && leg.type === 'move') {
            ...
            const loadEstimate = estimateSingleLeadQuote(loadScaledLead, {
                crewSize: baseEstimate.crewSize,
                truckCount: baseEstimate.truckCount,
            ...
            collectedPenalties.push(...loadEstimate.pricingBreakdown.penalties.map(penalty => ({
                ...penalty,
        ...
        const defaultDeliveryShare = deliveryLegs.length > 0
            ...
            : 0;
        const deliveryShareTotal = deliveryLegs.reduce((sum, deliveryLeg) => sum + clampInventorySharePct(deliveryLeg.inventorySharePct, defaultDeliveryShare), 0);
        let unloadSharePct = clampInventorySharePct(leg.inventorySharePct, deliveryLegs.length > 0 ? Math.max(10, 100 - deliveryShareTotal) : 100);
        ...
        const routeContext = buildLegRouteContext(leg, baseEstimate.pricingBreakdown.routeCategory);
        const legEstimate = estimateSingleLeadQuote(scaledLead, {
            crewSize: baseEstimate.crewSize,
            truckCount: baseEstimate.truckCount,
        ...
        const loadUnloadBufferHours = roundQuarterHour(routeContext.routeCategory === 'long-distance' ? 0 : (loadHours + unloadHours + penaltyHours) * 0.1);
        ...
        const operationalHours = roundQuarterHour(loadHours + unloadHours + operationalDriveHours + penaltyHours + driveBufferHours + loadUnloadBufferHours);
        const longDistanceOperationalBase = Number(legEstimate.longDistanceTruckCost || 0) +
        ...
        const shareNote = deliveryLegs.length > 0
        ...
        const legLabel = getLegDisplayLabel(leg, index);
        collectedPenalties.push(...legEstimate.pricingBreakdown.penalties.map(penalty => ({
            ...penalty,
            label: `${legLabel} — ${penalty.label}`,
        })));
        routeCategories.add(routeContext.routeCategory || baseEstimate.pricingBreakdown.routeCategory);
    ...
    const adjustmentBreakdown = adjustmentCategories
        ...
        category: entry.category,
        label: entry.label,
        hours: roundQuarterHour(collectedPenalties
            .filter(penalty => penalty.category === entry.category && !penalty.isFlagOnly)
            .reduce((sum, penalty) => sum + penalty.hours, 0)),
    }))
        .filter(entry => entry.hours > 0);
    const routeCategory = routeCategories.has('long-distance')
        ? 'long-distance'
        : routeCategories.has('medium')
            ? 'medium'
            : baseEstimate.pricingBreakdown.routeCategory;
    const pricingStatus = missingDestination ? 'provisional' : 'ready';
...
function estimateLeadQuote(lead, overrides, factors) {
    const legs = (overrides?.legs || []).filter(leg => !!(leg.originAddress || leg.originCity || leg.destAddress || leg.destCity || leg.label || leg.type));
    if (legs.length === 0) {
...
function computeQuoteTotals(lineItems, depositRate = 0.4, discountAmount = 0) {
...
function deriveInventoryMetrics(inventory) {
    const normalized = (0, move_policy_1.applyMovePolicyToInventory)(inventory, { enforceExclusion: true })
        .map(item => ({
        ...item,
...
function buildSalesSummary(leads, quotes) {
