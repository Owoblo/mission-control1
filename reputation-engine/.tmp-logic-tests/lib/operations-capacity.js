...
exports.BRANCH_CAPACITY_ESTIMATES = void 0;
exports.computeBranchCapacitySnapshot = computeBranchCapacitySnapshot;
exports.listCapacityConflicts = listCapacityConflicts;
const operations_1 = require("./operations");
exports.BRANCH_CAPACITY_ESTIMATES = {
...
function getJobCrewSize(job) {
    return Math.max(Number(job.quote?.crewSize || 0), Number(job.lead.assignedCrew?.length || 0), 0);
}
function getJobMoveDate(job) {
    return job.lead.moveDate || job.quote?.moveDate;
}
function getJobBranch(job) {
    return job.lead.branch;
}
function computeBranchCapacitySnapshot(jobs, branch, date) {
    if (!branch || !exports.BRANCH_CAPACITY_ESTIMATES[branch]) {
        return {
            status: 'unavailable',
            jobsBooked: 0,
            crewUsed: 0,
    ...
    const crewCapacity = exports.BRANCH_CAPACITY_ESTIMATES[branch].crew;
    const truckCapacity = exports.BRANCH_CAPACITY_ESTIMATES[branch].trucks;
    const crewUsed = dayJobs.reduce((sum, job) => sum + getJobCrewSize(job), 0);
    const trucksUsed = dayJobs.reduce((sum, job) => sum + Number((0, operations_1.getQuotedTruckCount)(job.lead, job.quote) || 0), 0);
...
function listCapacityConflicts(jobs) {
    const dates = Array.from(new Set(jobs.map(getJobMoveDate).filter(Boolean)));
    const conflicts = [];
    for (const date of dates) {
        const branches = Array.from(new Set(jobs
        ...
        for (const branch of branches) {
            ...
            if (crewOverage <= 0 && truckOverage <= 0)
                continue;
            conflicts.push({
                date,
                branch,
