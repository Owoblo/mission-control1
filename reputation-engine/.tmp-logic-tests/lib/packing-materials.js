...
exports.PACKING_MATERIAL_PRESETS = void 0;
exports.parseSizeInches = parseSizeInches;
exports.getTvBoxMaterialPresetForSize = getTvBoxMaterialPresetForSize;
exports.buildPackingMaterialsEstimate = buildPackingMaterialsEstimate;
// Seeded from current-ish U-Haul supply pricing. Adjust as vendor pricing changes.
exports.PACKING_MATERIAL_PRESETS = [
...
function parseSizeInches(value) {
    if (!value)
        return null;
...
function getTvBoxMaterialPresetForSize(value) {
    const inches = parseSizeInches(value);
    if (!inches)
        return exports.PACKING_MATERIAL_PRESETS.find(item => item.id === 'tv-box-55') || null;
    return (exports.PACKING_MATERIAL_PRESETS.find(item => typeof item.tvMinInches === 'number' &&
        typeof item.tvMaxInches === 'number' &&
        inches >= item.tvMinInches &&
        inches <= item.tvMaxInches) ||
        exports.PACKING_MATERIAL_PRESETS.find(item => item.id === 'tv-box-70') ||
        null);
}
...
function getPreset(id) {
    return exports.PACKING_MATERIAL_PRESETS.find(item => item.id === id) || null;
}
function normalizeInventoryName(item) {
    return (item.name || item.item || '').trim().toLowerCase();
}
function getIncludedInventory(inventory) {
    return inventory.filter(item => item.included !== false);
}
function getInventoryQuantity(item) {
    return Math.max(1, Number(item.qty || 1));
}
function countInventoryMatches(inventory, matcher) {
    return inventory.reduce((sum, item) => {
        const name = normalizeInventoryName(item);
        if (!name || !matcher(name, item))
            return sum;
...
function addLine(lines, presetId, quantity, note) {
    if (quantity <= 0)
        return;
...
function estimateStandardBoxes(input) {
    ...
    const inventoryBoxCount = countInventoryMatches(input.inventory, name => {
        if (!name.includes('box'))
            return false;
        if (name.includes('tv box') || name.includes('mirror box') || name.includes('wardrobe box'))
            return false;
        return true;
    });
...
function buildPackingMaterialsEstimate(input) {
    const includedInventory = getIncludedInventory(input.inventory);
    if (includedInventory.length === 0 && !input.estimatedBoxes)
        return null;
    ...
    const tvBoxCounts = new Map();
    const mirrorBoxCount = countInventoryMatches(includedInventory, name => (name.includes('mirror') || name.includes('picture') || name.includes('artwork') || name.includes('framed')) &&
        !name.includes('box'));
    const mattressBagCount = countInventoryMatches(includedInventory, name => (name.includes('mattress') || name.includes('bed')) &&
        !name.includes('bedside'));
    includedInventory.forEach(item => {
        const name = normalizeInventoryName(item);
        if (!name.includes('tv') || name.includes('tv box'))
            return;
