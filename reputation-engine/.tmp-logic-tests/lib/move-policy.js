...
exports.formatMovePolicyCategoryLabel = formatMovePolicyCategoryLabel;
exports.getMovePolicyFinding = getMovePolicyFinding;
exports.applyMovePolicyToInventory = applyMovePolicyToInventory;
exports.summarizeMovePolicy = summarizeMovePolicy;
const MOVE_POLICY_RULES = [
    {
        id: 'bathroom_vanity',
    ...
    },
    {
        ...
        customerNote: 'Hot tubs are not included. A separate specialty mover is required.',
        repNote: 'Saturn Star does not move hot tubs. Refer out or remove from quoted scope.',
        forceExclude: true,
    },
    {
        id: 'pool_table',
        ...
        customerNote: 'Pool tables are not included. A separate specialty mover is required.',
        repNote: 'Saturn Star does not move pool tables. Refer out or remove from quoted scope.',
        forceExclude: true,
    },
    {
        id: 'propane',
...
function buildPrimaryItemText(item) {
    return normalizePolicyText([
        item.name,
        item.item,
        item.size,
...
function buildItemLabel(item, fallback) {
    return (item.name || item.item || fallback).trim();
}
function matchRule(text, primaryText = text) {
    if (!text)
        return null;
    return MOVE_POLICY_RULES.find(rule => {
        if (rule.id === 'safe') {
            return (primaryText.includes('gun safe') ||
...
function formatMovePolicyCategoryLabel(category) {
    if (category === 'default_exclude')
...
function getMovePolicyFinding(item) {
...
function applyMovePolicyToInventory(inventory, options) {
...
function summarizeMovePolicy(inventory, options) {
    const findings = [];
