...
exports.buildConsultativeMovePlan = buildConsultativeMovePlan;
function hasLine(lines, pattern) {
    return lines.some(line => pattern.test(`${line.description} ${line.details || ''}`));
}
function buildConsultativeMovePlan(input) {
    ...
    if (lead?.originAddress || lead?.originCity)
        knownNow.push(`Pickup: ${lead.originAddress || lead.originCity}`);
    if (destinationAddressKnown)
        knownNow.push(`Destination: ${lead?.destAddress || 'address captured'}`);
    else if (destinationCityKnown)
        knownNow.push(`Destination market: ${lead?.destCity}`);
    if (lead?.moveDate && !dateFlexible)
        knownNow.push(`Move date: ${lead.moveDate}`);
    else if (dateFlexible)
        knownNow.push(`Timing target: ${lead?.moveDateFlexibleReason || 'date still flexible'}`);
    if (lead?.propertyType)
        knownNow.push(`Property type: ${lead.propertyType.replaceAll('_', ' ')}`);
    if (dateFlexible) {
    ...
    if (!factors.destinationTiming)
        questions.push('Do the sale and possession dates line up, or is there a gap between homes?');
    ...
    if (factors.packingPreference === 'partial_help' || factors.packingPreference === 'full_service')
        recommended.add('packing');
    ...
    if (storageNeeded && !storagePriced)
        assumptions.push('Storage handling and monthly storage still need pricing.');
    if (recommended.has('packing') && !packingPriced)
        assumptions.push('Requested packing help is not yet included in the price.');
    if (recommended.has('cleaning') && !cleaningPriced)
        assumptions.push('Requested cleaning is not yet included in the price.');
    if (recommended.has('protection') && !protectionPriced)
        assumptions.push('Enhanced protection still needs selection and pricing.');
    const phases = [
        {
            id: 'prepare',
            label: 'Prepare',
    ...
    const estimateMode = !dateFlexible && destinationAddressKnown && !unresolvedCommercialInputs
        ? 'firm'
