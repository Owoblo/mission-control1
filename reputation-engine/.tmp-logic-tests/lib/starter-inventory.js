...
exports.buildStarterInventoryPlan = buildStarterInventoryPlan;
exports.mergeStarterInventory = mergeStarterInventory;
const sales_1 = require("./sales");
const item_presets_1 = require("./item-presets");
const PRESET_BY_ID = new Map(item_presets_1.INVENTORY_PRESETS.map(preset => [preset.id, preset]));
function labelForBedroom(value) {
    return sales_1.PROPERTY_BEDROOM_OPTIONS.find(option => option.id === value)?.label || 'Home';
}
function labelForPropertyType(value) {
    return sales_1.PROPERTY_TYPE_OPTIONS.find(option => option.id === value)?.label || 'move';
}
function buildItems(specs) {
    return specs.flatMap(spec => {
        const preset = PRESET_BY_ID.get(spec.id);
        if (!preset)
            return [];
        const item = (0, item_presets_1.createInventoryItemFromPreset)(preset);
        item.qty = Math.max(1, Number(spec.qty || 1));
        return [item];
    });
}
function residentialPresetSpecs(bedrooms, propertyType) {
    const specs = [];
    const add = (id, qty = 1) => specs.push({ id, qty });
    if (bedrooms === 'studio') {
        add('sofa-loveseat');
        add('coffee-table-sm');
        add('tv-stand-sm');
...
function buildStarterInventoryPlan(input) {
...
function mergeStarterInventory(existing, starterItems) {
    const seen = new Set(existing.map(item => `${(item.room || '').trim().toLowerCase()}::${(item.name || item.item || '').trim().toLowerCase()}`));
