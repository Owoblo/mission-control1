"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MANAGER_MAX_DISCOUNT_PCT = exports.SALES_REP_MAX_DISCOUNT_PCT = void 0;
exports.isSalesWorkspaceRole = isSalesWorkspaceRole;
exports.canAccessSalesWorkspace = canAccessSalesWorkspace;
exports.isBranchScopedManager = isBranchScopedManager;
exports.leadMatchesSessionBranch = leadMatchesSessionBranch;
exports.canAccessOperationsWorkspace = canAccessOperationsWorkspace;
exports.canAssignCrew = canAssignCrew;
exports.isLeadOwnedBySession = isLeadOwnedBySession;
exports.canEditLead = canEditLead;
exports.canHandleLeadCommunications = canHandleLeadCommunications;
exports.canHandleLeadPayments = canHandleLeadPayments;
exports.canReassignLead = canReassignLead;
exports.canDeleteLead = canDeleteLead;
exports.canControlAutomation = canControlAutomation;
exports.canEditQuote = canEditQuote;
exports.canReviseExistingQuote = canReviseExistingQuote;
exports.validateQuotePricingPermissions = validateQuotePricingPermissions;
const sales_1 = require("../sales");
exports.SALES_REP_MAX_DISCOUNT_PCT = 0.1;
exports.MANAGER_MAX_DISCOUNT_PCT = 0.2;
function isSalesWorkspaceRole(role) {
    return role === 'owner' || role === 'manager' || role === 'sales_rep';
}
function canAccessSalesWorkspace(session) {
    return !!session && isSalesWorkspaceRole(session.role);
}
function isBranchScopedManager(session) {
    return session?.role === 'manager' && Boolean(session.branch);
}
function leadMatchesSessionBranch(lead, session) {
    if (!isBranchScopedManager(session))
        return true;
    const detected = lead.branch || (0, sales_1.detectSalesBranchFromLocation)(lead.originCity, lead.originAddress, lead.destCity, lead.destAddress);
    return detected === session?.branch;
}
function canAccessOperationsWorkspace(session) {
    return !!session && (session.role === 'owner' ||
        session.role === 'manager' ||
        session.role === 'operations_lead');
}
function canAssignCrew(session) {
    return !!session && (session.role === 'owner' ||
        session.role === 'manager' ||
        session.role === 'operations_lead');
}
function isLeadOwnedBySession(lead, session) {
    if (!session?.userId && !session?.name)
        return false;
    const ownerKey = (0, sales_1.getLeadAssignedRepKey)(lead);
    if (!ownerKey)
        return false;
    return ownerKey === session.userId || ownerKey === session.name || (0, sales_1.getLeadAssignedRepName)(lead) === session.name;
}
function canEditLead(session, lead) {
    if (!canAccessSalesWorkspace(session))
        return false;
    if (!leadMatchesSessionBranch(lead, session))
        return false;
    if (session?.role === 'owner' || session?.role === 'manager')
        return true;
    return !(0, sales_1.getLeadAssignedRepKey)(lead) || isLeadOwnedBySession(lead, session);
}
function canHandleLeadCommunications(session, lead) {
    return canAccessSalesWorkspace(session) && leadMatchesSessionBranch(lead, session);
}
function canHandleLeadPayments(session, lead) {
    return canEditLead(session, lead);
}
function canReassignLead(session) {
    return session?.role === 'owner' || session?.role === 'manager';
}
function canDeleteLead(session) {
    return session?.role === 'owner' || session?.role === 'manager';
}
function canControlAutomation(session, lead) {
    if (!canAccessSalesWorkspace(session))
        return false;
    if (!leadMatchesSessionBranch(lead, session))
        return false;
    if (session?.role === 'owner' || session?.role === 'manager')
        return true;
    return isLeadOwnedBySession(lead, session);
}
function canEditQuote(session, lead) {
    if (!canAccessSalesWorkspace(session))
        return false;
    if (lead && !leadMatchesSessionBranch(lead, session))
        return false;
    if (!lead)
        return session?.role === 'owner' || session?.role === 'manager';
    if (session?.role === 'owner' || session?.role === 'manager')
        return true;
    return !(0, sales_1.getLeadAssignedRepKey)(lead) || isLeadOwnedBySession(lead, session);
}
function canReviseExistingQuote(session) {
    return canAccessSalesWorkspace(session);
}
function deriveSubtotal(quote, updates) {
    if (typeof updates.subtotal === 'number' && Number.isFinite(updates.subtotal)) {
        return updates.subtotal;
    }
    if (Array.isArray(updates.lineItems)) {
        return updates.lineItems.reduce((sum, item) => sum + Number(item.amount || 0), 0);
    }
    return Number(quote.subtotal || 0);
}
function validateQuotePricingPermissions(session, current, updates) {
    if (!canAccessSalesWorkspace(session)) {
        return 'Unauthorized';
    }
    // Reps can apply healthy-margin overrides directly, but low/unknown margin needs an approval code.
    const overrideLineItem = Array.isArray(updates.lineItems)
        ? updates.lineItems.find(item => item.description === 'Moving Services — Agreed Rate')
        : null;
    if (session?.role === 'sales_rep' && overrideLineItem) {
        const overrideAmount = Math.round(Number(overrideLineItem.amount || 0) * 100) / 100;
        const approvedAmount = Math.round(Number(current.priceOverrideApprovalAmount || 0) * 100) / 100;
        const currentBaseAmount = Math.round(Number(current.subtotal || 0) * 100) / 100;
        const isUpwardOverride = currentBaseAmount > 0 && overrideAmount >= currentBaseAmount;
        const details = String(overrideLineItem.details || updates.priceOverrideReason || '');
        const marginMatch = details.match(/Projected margin:\s*(-?\d+(?:\.\d+)?)%/i);
        const projectedMargin = marginMatch ? Number(marginMatch[1]) : null;
        const hasApproval = current.priceOverrideApprovalStatus === 'approved' &&
            approvedAmount > 0 &&
            approvedAmount === overrideAmount;
        const hasMeaningfulNote = details.replace(/Projected margin:.*$/i, '').trim().length >= 12;
        if (!hasMeaningfulNote) {
            return 'Sales reps must add a quick note explaining every manual price override.';
        }
        // Raising the base price cannot create the discount/margin risk this gate is
        // designed to prevent. Keep the audit note, but never block an upward revision.
        if (!isUpwardOverride && (projectedMargin === null || projectedMargin < 55) && !hasApproval) {
            return 'Sales reps need an owner/manager approval code before applying a manual price override.';
        }
    }
    if (updates.discountAmount !== undefined) {
        const subtotal = deriveSubtotal(current, updates);
        const discountAmount = Math.max(0, Number(updates.discountAmount || 0));
        const discountPct = subtotal > 0 ? discountAmount / subtotal : 0;
        if (session?.role === 'sales_rep' && discountPct > exports.SALES_REP_MAX_DISCOUNT_PCT) {
            return 'Sales reps can apply up to a 10% discount.';
        }
        if (session?.role === 'manager' && discountPct > exports.MANAGER_MAX_DISCOUNT_PCT) {
            return 'Managers can apply up to a 20% discount.';
        }
    }
    return null;
}
