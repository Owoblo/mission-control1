...
function inferMarketKey(contact) {
    const text = `${contact.city || ''} ${contact.company || ''} ${contact.tracking_code || ''}`.toLowerCase();
    if (/\b(london|st\.?\s*thomas|woodstock|strathroy|sarnia|ingersoll|tillsonburg)\b/.test(text))
        return 'london';
    if (/\b(kitchener|waterloo|cambridge|kw|elmira|brantford|new hamburg|ayr)\b/.test(text))
        return 'waterloo';
    if (/\b(guelph|fergus|elora|wellington)\b/.test(text))
        return 'guelph';
    if (/\b(chatham)\b/.test(text))
        return 'chatham';
    return 'windsor';
}
function getPackageConfig(contact) {
    const baseReferralUrl = (0, runtime_1.readEnv)('PARTNERSHIP_REFERRAL_PROGRAM_URL');
    ...
    return {
        digitalPackageUrl: (0, runtime_1.readEnv)('PARTNERSHIP_DIGITAL_PACKAGE_URL') || defaultPackageUrl,
        rateCardUrl: (0, runtime_1.readEnv)('PARTNERSHIP_RATE_CARD_URL') || defaultPackageUrl,
        referralProgramUrl,
        flyerImageUrl: (0, runtime_1.readEnv)('PARTNERSHIP_FLYER_IMAGE_URL') || defaultFlyerUrl,
        dedicatedPhone: (0, runtime_1.readEnv)('PARTNERSHIP_DEDICATED_PHONE') || (0, partnership_lines_1.getPartnershipPrimaryNumberForMarket)(marketKey),
        packageSlug,
        marketKey,
    };
}
...
function latestInbound(touches) {
    return [...touches]
        .sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)))
        .find(touch => touch.direction === 'inbound' && cleanText(touch.notes));
}
function wasSent(touches, pattern) {
    return touches.some(touch => {
        if (touch.direction !== 'outbound' && touch.direction !== 'system')
            return false;
        return pattern.test(cleanText(touch.notes));
    });
}
function latestEmailInHistory(touches) {
    return [...touches]
        .sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)))
...
function packagePermissionGranted(touches, latestText, intent) {
    if (['asks_for_email', 'asks_for_pricing', 'asks_referral_program', 'send_digital_package', 'asks_for_references'].includes(intent))
        return true;
    if (intent === 'confirms_identity' && priorInboundApprovedDigitalPackage(touches))
        return true;
    if (DIRECT_PACKAGE_REQUEST_RE.test(latestText) && !CARD_OR_FLYER_REQUEST_RE.test(latestText))
        return true;
...
function priorInboundApprovedDigitalPackage(touches) {
    return [...touches]
        .sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)))
        .slice(0, 8)
        .some(touch => {
...
function priorOutboundAskedCardDrop(touches) {
    return [...touches]
        .sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)))
        .slice(0, 8)
        .some(touch => {
...
function detectIntent(text, contact, touches = []) {
    ...
    if (/\b(wrong number|wrong person|not me)\b/i.test(text))
        return { intent: 'wrong_number', confidence: 0.96, risk_flags };
    if (stage === 'closed_lost' || /\b(not interested|no thanks|no thank you|remove me|don't contact|do not contact)\b/i.test(text))
        return { intent: 'not_interested', confidence: 0.92, risk_flags };
    ...
    if (IDENTITY_CONFIRMATION_RE.test(text))
    ...
    if (REFERENCES_RE.test(text) && /\b(add|include|send|share|have|provide|couple)\b/i.test(text))
        return { intent: 'asks_for_references', confidence: 0.84, risk_flags };
    if (/\b(price|prices|pricing|rate|rates|charge|cost|fee)\b/i.test(text))
        return { intent: 'asks_for_pricing', confidence: 0.88, risk_flags };
    if (/\b(referral|commission|incentive|program|kickback|paid)\b/i.test(text))
    ...
    if (EMAIL_RE.test(text) || /\b(send it over|send me.*link|website link)\b/i.test(text) || /\b(email|e-mail)\b/i.test(text) && /\b(send|share|forward|text|package|info|information|link|card|flyer|rates?)\b/i.test(text))
        return { intent: 'asks_for_email', confidence: 0.86, risk_flags };
    if (ADDRESS_RE.test(text))
        return { intent: 'gives_address', confidence: 0.9, risk_flags };
    if (/\b(meet|meeting|appointment|call me|give me a call|phone call|sit down|come by)\b/i.test(text))
        return { intent: 'wants_meeting', confidence: 0.84, risk_flags };
    if (TIME_RE.test(text) && /\b(drop|stop|come|available|free|works|week|time|between)\b/i.test(text))
        return { intent: 'gives_time_window', confidence: 0.82, risk_flags };
    if (/\b(drop by|stop by|drop off|leave (?:it|them)|mailbox|front desk|reception|secretary)\b/i.test(text))
        return { intent: 'drop_by_anytime', confidence: 0.82, risk_flags };
    ...
    if (/\b(postcard|post card|card|cards|flyer|flyers|brochure|package|send.*info|sure|of course|ok|okay|yes|awesome|sounds good)\b/i.test(text))
        return { intent: 'postcard_yes', confidence: 0.78, risk_flags };
    if (/\b(interested|tell me more|send|share|forward)\b/i.test(text))
        return { intent: 'send_digital_package', confidence: 0.72, risk_flags };
    if (lower.length <= 24)
        risk_flags.push('short_or_ambiguous_reply');
    return { intent: 'positive_vague', confidence: lower.length <= 24 ? 0.58 : 0.64, risk_flags };
}
function packageLine(config, extracted) {
    const parts = [];
    if (config.digitalPackageUrl)
        parts.push(`digital package: ${config.digitalPackageUrl}`);
    if (config.referralProgramUrl)
        parts.push(`referral program: ${config.referralProgramUrl}`);
    if (extracted.asks_pricing && config.rateCardUrl)
        parts.push(`rate card: ${config.rateCardUrl}`);
    return parts.length ? `I can send over the ${parts.join(' and ')}.` : 'I can send the digital package over from here.';
}
function packagePermissionAsk(extracted) {
...
function publicContactEmail() {
    return (0, runtime_1.readEnv)('PARTNERSHIP_PUBLIC_EMAIL') || (0, runtime_1.readEnv)('PARTNERSHIP_CONTACT_EMAIL') || 'info@starmovers.ca';
}
function publicWebsite() {
    return (0, runtime_1.readEnv)('PARTNERSHIP_PUBLIC_WEBSITE') || 'starmovers.ca';
}
...
function draftFromRules(input) {
    ...
    const risk_flags = [];
    let draft = '';
    let recommended_action = 'draft_reply';
    let quick_action;
...
function normalizeAiResult(value, fallback, canSendPackageNow, config) {
    const allowedActions = ['draft_reply', 'schedule_delivery', 'book_meeting', 'send_package', 'mark_not_interested', 'human_review'];
    const allowedQuickActions = ['active_partner', 'drop_cards', 'meeting_requested', 'needs_follow_up', 'not_interested', 'wrong_number'];
    ...
    const noUnsafeLinks = draft.replace(/https?:\/\/\S+/gi, link => {
        const allowed = [
            ...
            (0, runtime_1.readEnv)('PARTNERSHIP_DIGITAL_PACKAGE_URL'),
            (0, runtime_1.readEnv)('PARTNERSHIP_RATE_CARD_URL'),
            (0, runtime_1.readEnv)('PARTNERSHIP_REFERRAL_PROGRAM_URL'),
            (0, runtime_1.readEnv)('PARTNERSHIP_FLYER_IMAGE_URL'),
        ].filter(Boolean);
    ...
    if (/\b(ai|automated|bot|chatgpt)\b/i.test(noUnsafeLinks))
        risk_flags.push('mentions_automation');
    if (!canSendPackageNow && /https?:\/\//i.test(draft))
        risk_flags.push('removed_unrequested_link');
    return {
        ...fallback,
...
async function refineWithOpenAi(input) {
    const apiKey = (0, runtime_1.readEnv)('OPENAI_API_KEY');
    if (!apiKey || ['stop_opt_out', 'wrong_number', 'not_interested'].includes(input.fallback.intent))
        return input.fallback;
    const model = (0, runtime_1.readEnv)('OPENAI_AUTOMATION_MODEL') || 'gpt-4o-mini';
    ...
    try {
        ...
        const parsed = JSON.parse(extractJsonObject(payload.choices?.[0]?.message?.content || '{}'));
...
async function suggestPartnershipReply(input) {
...
function partnershipDispositionFromSuggestion(result) {
