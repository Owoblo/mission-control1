...
exports.sendCallerIdSms = sendCallerIdSms;
exports.getPartnershipAlertRecipients = getPartnershipAlertRecipients;
exports.sendInternalAlertSms = sendInternalAlertSms;
exports.sendRepAlertEmail = sendRepAlertEmail;
exports.sendRepAlertEmailWithAttachments = sendRepAlertEmailWithAttachments;
exports.buildDepositPaidAlertEmail = buildDepositPaidAlertEmail;
exports.sendDepositPaidAlert = sendDepositPaidAlert;
exports.partnershipInboundNotificationEmail = partnershipInboundNotificationEmail;
exports.sendPartnershipInboundAlert = sendPartnershipInboundAlert;
exports.smsNotificationEmail = smsNotificationEmail;
exports.voicemailNotificationEmail = voicemailNotificationEmail;
exports.missedCallNotificationEmail = missedCallNotificationEmail;
exports.quoteViewedEmail = quoteViewedEmail;
exports.quoteAcceptedEmail = quoteAcceptedEmail;
exports.surveyPhotosUploadedEmail = surveyPhotosUploadedEmail;
exports.surveyCompletedEmail = surveyCompletedEmail;
exports.depositReceivedEmail = depositReceivedEmail;
const runtime_1 = require("@/lib/server/runtime");
const twilio_recordings_1 = require("@/lib/server/twilio-recordings");
// Per-caller cooldown so reps don't get spammed on repeat calls
const _callerIdCooldown = new Map();
const CALLER_ID_COOLDOWN_MS = 5 * 60 * 1000;
async function sendCallerIdSms(callerPhone, leadName, branchLabel, repPhones, businessNumber) {
    if (!repPhones.length)
        return;
    // Dedup: don't send again within 5 min for same caller
    const last = _callerIdCooldown.get(callerPhone) || 0;
    if (Date.now() - last < CALLER_ID_COOLDOWN_MS)
        return;
    _callerIdCooldown.set(callerPhone, Date.now());
    const { accountSid, authToken } = (0, runtime_1.getTwilioCredentials)();
    const message = `📞 ${leadName} is calling · ${callerPhone} · ${branchLabel}`;
    await Promise.all(repPhones.map(phone => fetch(`https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`, {
        method: 'POST',
        headers: {
            Authorization: (0, twilio_recordings_1.twilioAuth)(accountSid, authToken),
...
const PARTNERSHIP_MARKET_NOTIFY_TO = {
...
function getPartnershipAlertRecipients(market) {
    ...
    return uniqueEmails([
        ...
        ...marketRecipients,
        (0, runtime_1.readEnv)('PARTNERSHIP_NOTIFY_TO'),
    ]);
}
...
async function sendInternalAlertSms(to, body, from) {
    const { accountSid, authToken } = (0, runtime_1.getTwilioCredentials)();
...
async function sendRepAlertEmail(subject, htmlBody, recipients = NOTIFY_TO) {
    return sendRepAlertEmailWithAttachments(subject, htmlBody, recipients);
}
async function sendRepAlertEmailWithAttachments(subject, htmlBody, recipients = NOTIFY_TO, attachments = []) {
    const resendKey = (0, runtime_1.readEnv)('RESEND_API_KEY');
...
function buildDepositPaidAlertEmail(input) {
    ...
    const sourceLabel = input.source === 'phone_card' ? 'Card taken by phone' :
...
async function sendDepositPaidAlert(input) {
    const email = buildDepositPaidAlertEmail(input);
    return sendRepAlertEmail(email.subject, email.html);
}
function partnershipInboundNotificationEmail(options) {
    const { contactId, contactName, company, channel, occurredAt, notes, phone, email, mediaUrls = [], embeddedMedia = [], } = options;
...
async function downloadPartnershipMediaForEmail(mediaUrls) {
    const accountSid = (0, runtime_1.readEnv)('TWILIO_ACCOUNT_SID');
    const authToken = (0, runtime_1.readEnv)('TWILIO_AUTH_TOKEN');
    ...
    const downloaded = await Promise.all(mediaUrls.slice(0, 10).map(async (mediaUrl, index) => {
        try {
            ...
            const response = await fetch(parsed.toString(), {
                headers: { Authorization: (0, twilio_recordings_1.twilioAuth)(accountSid, authToken) },
                cache: 'no-store',
            ...
            if (declaredLength > MAX_PARTNERSHIP_MEDIA_BYTES)
                return null;
            const contentType = (response.headers.get('content-type') || 'image/jpeg').split(';')[0].trim().toLowerCase();
            if (!contentType.startsWith('image/'))
                return null;
            const buffer = Buffer.from(await response.arrayBuffer());
            if (!buffer.length || buffer.length > MAX_PARTNERSHIP_MEDIA_BYTES)
                return null;
    ...
    let totalBytes = 0;
    return downloaded.filter((item) => {
        if (!item || totalBytes + item.bytes > MAX_PARTNERSHIP_EMAIL_MEDIA_BYTES)
            return false;
        totalBytes += item.bytes;
        return true;
    });
}
async function sendPartnershipInboundAlert(subject, options, recipients) {
    ...
    return sendRepAlertEmailWithAttachments(subject, html, recipients, media.map(item => item.attachment));
}
function smsNotificationEmail(from, body, leadId) {
...
function voicemailNotificationEmail(from, duration, recordingUrl, leadId) {
    const crmLink = leadId
...
function missedCallNotificationEmail(from, branchLabel, leadId) {
    const crmLink = leadId
        ? `https://mission-control1-reputation-engine.vercel.app/sales/leads/${leadId}`
        : `https://mission-control1-reputation-engine.vercel.app/sales/leads`;
    return `
...
function quoteViewedEmail(leadName, leadId, quoteNumber) {
...
function quoteAcceptedEmail(leadName, leadId, quoteNumber, total) {
    return customerEventEmail({
        emoji: '✅',
...
function surveyPhotosUploadedEmail(leadName, leadId, photoCount, room) {
    return customerEventEmail({
        emoji: '📸',
...
function surveyCompletedEmail(leadName, leadId, photoCount) {
    return customerEventEmail({
        emoji: '📋',
...
function depositReceivedEmail(leadName, leadId, amount, quoteNumber) {
    return customerEventEmail({
        emoji: '💰',
