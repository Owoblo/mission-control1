...
exports.extractStructuredInboundLeadFields = extractStructuredInboundLeadFields;
exports.extractDeterministicReplyFields = extractDeterministicReplyFields;
...
function normalizeFormBedrooms(value) {
    const count = Number(value.match(/\d+/)?.[0]);
    if (/\bstudio\b/i.test(value))
        return 'studio';
    if (count === 1)
        return '1_bedroom';
    if (count === 2)
 ...
 */
function extractStructuredInboundLeadFields(raw, message) {
    const text = String(message || '');
    const originAddress = rawText(raw, 'move_from', 'moveFrom', 'originAddress', 'origin_address', 'pickup_address') ||
        text.match(/(?:^|\|)\s*(?:from|pickup|origin)\s*:\s*(.*?)(?=\s*\||$)/i)?.[1]?.trim() ||
        '';
    const destAddress = rawText(raw, 'move_to', 'moveTo', 'destAddress', 'destination_address', 'dropoff_address') ||
        text.match(/(?:^|\|)\s*(?:to|dropoff|destination)\s*:\s*(.*?)(?=\s*\||$)/i)?.[1]?.trim() ||
        '';
    const moveDateText = rawText(raw, 'move_date', 'moveDate', 'date') ||
        text.match(/(?:^|\|)\s*(?:move\s*)?date\s*:\s*(.*?)(?=\s*\||$)/i)?.[1]?.trim() ||
        '';
    const homeSize = rawText(raw, 'home_size', 'homeSize', 'propertyBedrooms', 'bedrooms') ||
        text.match(/(?:^|\|)\s*(?:home|property)\s*size\s*:\s*(.*?)(?=\s*\||$)/i)?.[1]?.trim() ||
        '';
    const serviceType = rawText(raw, 'service_type', 'serviceType', 'service') ||
        text.match(/(?:^|\|)\s*service\s*:\s*(.*?)(?=\s*\||$)/i)?.[1]?.trim() ||
        '';
    const notes = rawText(raw, 'message', 'notes', 'additional_notes') ||
    ...
    return {
        ...
        ...(/\b(local|residential)\b/i.test(serviceType) ? { moveType: 'residential' } : {}),
        ...(hasStairs ? {
            originAccess: 'Stairs reported on website form; flight count to confirm',
        } : {}),
    };
}
function extractDeterministicReplyFields(message) {
...
function matchCustomerInventoryPreset(name) {
    ...
    if (/\b(?:recliner|reclining|lazy\s*boy|la-z-boy)\b.*\b(?:couch|sofa)\b|\b(?:couch|sofa)\b.*\b(?:recliner|reclining)\b/i.test(name)) {
        return item_presets_1.INVENTORY_PRESETS.find(preset => preset.id === 'sofa-standard');
    }
    const aliases = name
    ...
    return (0, item_presets_1.matchInventoryPreset)(aliases);
}
function parseInventoryCandidate(value) {
    const trimmed = value
        .replace(/^(?:>+\s*)+/, '')
        .replace(/^[•●▪◦*-]\s*/, '')
        ...
        .trim();
    const quantityMatch = trimmed.match(new RegExp(`^(one|two|three|four|five|six|seven|eight|nine|ten)\\b\\s+|^(\\d{1,2})\\s*[x×]\\s*|^(\\d{1,2})\\s+(?=${COUNTABLE_ITEM_TOKEN}\\b)`, 'i'));
    ...
    return {
        ...
        status: uncertainDisposition ? 'needs_confirmation' : undefined,
