...
export default function ListingActivityPage() {
 ...
 async function load(){setLoading(true);try{const r=await fetch('/api/marketing/listing-activity');if(!r.ok)throw new Error('Unable to load listing activity.');const d=await r.json();setGroups(d.groups);setResearch(d.research||{});setError('')}catch(e){setError((e as Error).message)}finally{setLoading(false)}}
 useEffect(()=>{void load()},[])
 ...
 return <main className="mx-auto max-w-7xl p-4 sm:p-6">
  ...
  <div className="my-5 flex flex-wrap gap-3">
   ...
   <Link href="/marketing/listing-activity/assessments" className="rounded-lg border p-2">Run assessments</Link>
  </div>
  ...
  {loading?<p>Loading activity…</p>:<div className="grid gap-5 lg:grid-cols-[340px_1fr]">
