...
export async function GET(request: Request) {
  ...
  for (let i = 0; i < contactIds.length; i += 100) {
    const response = await fetch(`${url}/rest/v1/market_contacts?select=id,name,company,phone,email,stage,last_touch_at,last_inbound_at,do_not_contact,sequence_paused,listing_discovery_key&id=in.(${contactIds.slice(i,i+100).join(',')})`, { headers, cache: 'no-store' })
  ...
  const contactMap = new Map(contacts.map(c => [c.id,c])), batchMap=new Map(batches.map(b=>[b.batch_id,b]))
...
export async function PATCH(request: Request) {
  ...
  if (body.create_contact === true) {
    if (!row.representative || typeof row.representative !== 'object' || typeof row.representative.name !== 'string' || !row.representative.name.trim()) {
      return NextResponse.json({ error: 'Listing source is missing a representative name; review the source before creating a contact.' }, { status: 422 })
    }
    const rep = row.representative as Record<string, unknown>
