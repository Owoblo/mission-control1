...
function classifyReply(touch: MarketTouch, contact?: MarketContact | null) {
  ...
  if (outcome === 'polite_decline') return 'responded'
  if (['opt_out', 'wrong_number'].includes(outcome)) return 'opt_out'
  ...
  if (['package_requested', 'media_requested', 'digital_only', 'asks_contact_info', 'asks_for_email', 'asks_pricing', 'asks_referral_program', 'asks_social_media', 'asks_references', 'secondary_contact_referral', 'partner_lead_received', 'lead_disposition_update'].includes(outcome)) return 'needs_reply'
