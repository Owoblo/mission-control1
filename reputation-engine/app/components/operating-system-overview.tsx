  ...
  return (
    <section className="space-y-8 border-b border-[var(--app-line)] pb-10">
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
        ...
        <aside>
          <div className="flex items-baseline justify-between border-b border-[var(--app-line)] pb-3"><div><h2 className="text-xl font-semibold text-[#071421]">Attention needed</h2><p className="mt-1 text-xs text-[var(--app-muted)]">Exceptions become accountable team tasks</p></div><div className="flex items-center gap-3"><Link href="/sales/tasks" className="text-xs font-semibold text-[#8a6800] hover:underline">Open Tasks</Link><span className={`text-sm font-semibold ${exceptions.some(item => item.severity === 'urgent') ? 'text-rose-700' : 'text-[var(--app-muted)]'}`}>{exceptions.length}</span></div></div>
          <div className="divide-y divide-[var(--app-line)]">
