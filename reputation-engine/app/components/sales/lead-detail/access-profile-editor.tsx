...
export function AccessProfileEditor({ lead, factors, legs, singleLocation, stopRole, compact = false, baseHours, currentUserName, onChange }: { lead: CRMLead; factors: JobFactors; legs?: QuoteLeg[]; singleLocation?: boolean; stopRole?: 'pickup' | 'dropoff'; compact?: boolean; baseHours: { origin: number; destination: number }; currentUserName?: string; onChange: (next: JobFactors) => void }) {
  ...
  function choosePropertyType(profile: AccessProfile, propertyType: AccessPropertyType) {
    ...
    patch(profile, {
      propertyType,
      propertyTypeSource: 'rep_selected',
  ...
  function confirmTypicalHouse(profile: AccessProfile) {
    saveProfile({
      ...
      propertyType: profile.propertyType || inferredPropertyType(profile) || 'detached',
      propertyTypeSource: profile.propertyTypeSource || 'rep_selected',
      entranceLocation: 'front',
      evidenceStatus: 'customer_confirmed',
      evidenceNote: 'Customer confirmed driveway or nearby legal parking, a short ground-floor carrying route, and no unusual building access.',
    })
      ...
      {profiles.map(profile => {
        ...
        return <div key={profile.stopId} className="rounded-[8px] border border-[var(--app-line)] bg-[var(--app-bg)] p-3">
          ...
          {open ? <div className="mt-3 grid gap-3 border-t border-[var(--app-line)] pt-3 sm:grid-cols-2">
            ...
            {profile.verticalMode?.includes('elevator') || profile.verticalMode === 'stairs_or_elevator' ? <><Select label="Elevator used for the move" value={profile.elevatorType} options={([['freight', 'Dedicated freight / cargo elevator'], ['passenger', 'Regular passenger elevator'], ['unknown', 'Not sure yet']] as Array<[NonNullable<AccessProfile['elevatorType']>, string]>)} onChange={value => patch(profile, { elevatorType: value })}/><label className="text-[10px] font-semibold text-[var(--app-muted)]">How many usable elevators?<input type="number" min="1" className="crm-input mt-1 w-full py-1.5 text-xs" value={profile.elevatorCount ?? ''} onChange={event => patch(profile, { elevatorCount: Number(event.target.value || 0) || undefined })}/></label><Select label="Can the crew reserve it?" value={profile.elevatorReservation} options={([['confirmed', 'Yes — reservation confirmed'], ['requested', 'Requested, awaiting confirmation'], ['shared', 'No — shared with residents'], ['not_available', 'No usable elevator'], ['unknown', 'Need to ask building']] as Array<[NonNullable<AccessProfile['elevatorReservation']>, string]>)} onChange={value => patch(profile, { elevatorReservation: value })}/><Select label="Crew access during the move" value={profile.elevatorExclusive === undefined ? undefined : profile.elevatorExclusive ? 'yes' : 'no'} options={([['yes', 'Exclusive for the crew'], ['no', 'Shared during the move']] as Array<['yes' | 'no', string]>)} onChange={value => patch(profile, { elevatorExclusive: value === 'yes' })}/><Select label="Typical elevator wait" value={profile.elevatorWait} options={([['short', 'Usually under 2 minutes'], ['normal', 'Usually 2–5 minutes'], ['slow', 'Usually 5–10 minutes'], ['likely_delays', 'Frequent delays expected'], ['unknown', 'Need to confirm']] as Array<[NonNullable<AccessProfile['elevatorWait']>, string]>)} onChange={value => patch(profile, { elevatorWait: value })}/></> : null}
            ...
            <div className="sm:col-span-2 rounded-[7px] border border-[var(--app-line)] bg-white p-3"><div className="mb-2 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--app-muted)]">Anything else the crew must plan for?</div><div className="flex flex-wrap gap-3 text-[10px] text-slate-700">{([['narrowDoor', 'Narrow door'], ['tightTurn', 'Tight turn'], ['buildingCheckIn', 'Security/check-in'], ['elevatorPadding', 'Elevator padding required'], ['multipleTransfers', 'More than one elevator/transfer'], ['shuttleMayBeRequired', 'Smaller shuttle vehicle may be needed'], ['unsafeAccess', 'Steep, icy, or unsafe access']] as Array<[keyof AccessProfile, string]>).map(([key, label]) => <label key={key} className="flex items-center gap-1"><input type="checkbox" checked={Boolean(profile[key])} onChange={event => patch(profile, { [key]: event.target.checked } as Partial<AccessProfile>)}/>{label}</label>)}</div></div>
          </div> : null}
