...
export default function EconomicTruthPage() {
  ...
  const filtered = useMemo(() => rows.filter(row => {
    ...
    return matchesSearch && (view === 'all' || (view === 'completed' && ['completed', 'customer_success'].includes(row.stage)) ||
      (view === 'partner' && row.partnerId) || (view === 'action' && !['completed', 'customer_success', 'lost'].includes(row.stage) && (!row.owner || !row.nextAction || !row.dueAt || !Number.isFinite(Date.parse(row.dueAt)))))
  }), [rows, query, view])
  ...
  return <div className="crm-shell space-y-6">
    ...
    <div className="flex flex-wrap gap-3">
      ...
      <label className="text-sm">Review<select aria-label="Review filter" className="ml-2 rounded-lg border p-2" value={view} onChange={e => setView(e.target.value)}>
        <option value="all">All records</option><option value="completed">Completed jobs</option><option value="partner">Explicit partner referrals</option><option value="action">Missing action details</option>
      </select></label>
